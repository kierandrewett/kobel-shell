use freya_core::{
    integration::*,
    prelude::{
        Border, Color, CornerRadius, Fill, FontSlant, Shadow, TextAlign, TextDecoration, TextHeightBehavior,
...
impl NodeState {
    pub fn layout_attributes(&'_ self) -> Vec<(&'_ str, AttributeType<'_>)> {
        vec![
            ...
            ("min_height", AttributeType::Size(&self.layout.minimum_height)),
            ("max_width", AttributeType::Size(&self.layout.maximum_width)),
            ("max_height", AttributeType::Size(&self.layout.maximum_height)),
            ("visible_width", AttributeType::VisibleSize(&self.layout.visible_width)),
            (
                "visible_height",
                AttributeType::VisibleSize(&self.layout.visible_height),
            ),
            ("direction", AttributeType::Direction(&self.layout.direction)),
            ...
            ("main_alignment", AttributeType::Alignment(&self.layout.main_alignment)),
            (
                "cross_alignment",
                AttributeType::Alignment(&self.layout.cross_alignment),
            ),
            ("offset_x", AttributeType::Measure(self.layout.offset_x.get())),
            ("offset_y", AttributeType::Measure(self.layout.offset_y.get())),
    ...
    pub fn text_style_attributes(&'_ self) -> Vec<(&'_ str, AttributeType<'_>)> {
        let mut attributes = vec![
            ...
            ("text_align", AttributeType::TextAlignment(&self.text_style.text_align)),
            (
                "text_overflow",
            ...
            ("font_slant", AttributeType::FontSlant(self.text_style.font_slant)),
            (
                "font_weight",
                AttributeType::Measure(self.text_style.font_weight.into()),
            ),
            ("font_width", AttributeType::Measure(self.text_style.font_width.into())),
            (
                "text_decoration",
