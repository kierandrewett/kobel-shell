...
use crate::{
    DevtoolsPlugin, IncomingMessage, OutgoingMessage, OutgoingMessageAction, WindowState,
...
fn devtools_addr() -> String {
    env::var("FREYA_DEVTOOLS_ADDR").unwrap_or_else(|_| DEFAULT_DEVTOOLS_ADDR.to_owned())
}
                ...
                if let Ok(incoming) = incoming {
                    match incoming.action {
                        IncomingMessageAction::HighlightNode { window_id, node_id } => {
                            ...
                            plugin_handle.send_event_loop_event(NativeEvent::Window(NativeWindowEvent {
                        ...
                        IncomingMessageAction::SetSpeedTo { speed } => {
                            for WindowState { animation_clock, .. } in plugin.windows.lock().unwrap().values() {
...
pub(crate) async fn run_server(plugin: DevtoolsPlugin, plugin_handle: PluginHandle) -> anyhow::Result<()> {
    let address = devtools_addr();
    println!("Running the Devtools Server on {address}");
