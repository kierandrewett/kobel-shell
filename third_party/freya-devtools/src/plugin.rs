...
use crate::{NodeState, OutgoingMessage, OutgoingMessageAction, node_info::NodeInfo, server::run_server};
...
impl DevtoolsPlugin {
    ...
    pub fn init(&mut self, window_id: WindowId, animation_clock: &AnimationClock, plugin_handle: PluginHandle) {
        let start_server = {
    ...
    pub fn sync(&mut self, window_id: WindowId, scale_factor: f32, tree: &Tree) {
        ...
        tree.traverse_depth(|node_id| {
            ...
            let children_len = tree.children.get(&node_id).map(|c| c.len()).unwrap_or_default();
        ...
        match event {
            ...
            PluginEvent::AfterRender {
                ...
                if let Some(layout_node) = highlighted_node.and_then(|n| tree.layout.get(&n)) {
                    ...
                    Self::draw_wireframe(canvas, &area, Color::from_rgb(255, 192, 203), Color::GREEN);
                }
