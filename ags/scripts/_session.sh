...
KOBEL_ICONS="/home/kieran/dev/kobel-shell/ags/icons" KOBEL_SKIP_NOTIFD="$SKIP" KOBEL_DRILL="${KOBEL_DRILL:-}" stdbuf -oL -eL env LD_PRELOAD="$LAYER_PRELOAD" gjs -m "$BUNDLE" >"$DK/ags.log" 2>&1 &
AP=$!
sleep 8
if [ -n "${KOBEL_TEST_NOTIFD:-}" ]; then
