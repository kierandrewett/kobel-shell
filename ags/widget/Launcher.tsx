        ...
        const actRows: Row[] = ACTIONS.map((x) => {
            ...
            if (!m)
                for (const al of x.al) {
                    ...
                    if (am) {
                        m = { score: am.score - 0.5, marks: null as any }
                        break
                    }
                }
            return m
        ...
        <window
            ...
            onKeyPressed={(_self, key, _code, mods) => {
                ...
                if (key === Gdk.KEY_Tab) {
                    ...
                    const g = ghost.get(),
                        q = query.get()
                    if (g && !(mods & Gdk.ModifierType.SHIFT_MASK)) {
                        query.set(g)
                        return true
                    }
                    selected.set(
                        (selected.get() +
                            (mods & Gdk.ModifierType.SHIFT_MASK ? -1 : 1) +
                            flat.length) %
                ...
                if (
                    ...
                    selected.set(
                        (selected.get() + (key === Gdk.KEY_n ? 1 : -1) + flat.length) %
                            Math.max(flat.length, 1)
                    )
                    return true
                }
                        ...
                        {sections.as((secs) =>
                            secs.flatMap((sec) => [
                                ...
                                ...sec.rows.map((r) => {
                                    ...
                                    return (
                                        <button
                                            class={bind(selected).as((s) =>
                                                s === flatIdx ? "row sel" : "row"
                                            )}
                                            onClicked={() => {
                                                r.run()
                                                launchClose()
