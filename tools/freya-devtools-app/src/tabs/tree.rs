...
use freya_radio::prelude::{Radio, use_radio};
use freya_router::prelude::RouterContext;

use crate::{
    Route,
    node::NodeElement,
    state::{DevtoolsChannel, DevtoolsState},
};
                        ...
                        .filter_map(|node| {
                            ...
                            if parent_is_open || is_top_height {
                                ...
                                let is_open = (node.children_len != 0)
                                    .then_some(radio.expanded_nodes.contains(&(*window_id, node.node_id)));
                                Some(NodeTreeItem {
                                    is_open,
                ...
                NodeElement {
                    ...
                    on_selected: EventHandler::new(move |_| {
                        ...
                        let route = match RouterContext::get().current::<Route>() {
                            ...
                            Route::NodeInspectorLayout { .. } => Route::NodeInspectorLayout { node_id, window_id },
