...
use tabs::{computed_layout::computed_layout, layout::*, misc::*, style::*, text_style::*, tree::*};

const DEFAULT_DEVTOOLS_ADDR: &str = "127.0.0.1:7354";

fn devtools_addr() -> String {
    std::env::var("FREYA_DEVTOOLS_ADDR").unwrap_or_else(|_| DEFAULT_DEVTOOLS_ADDR.to_owned())
}
fn main() {
    launch(
...
pub fn app() -> impl IntoElement {
    ...
    use_hook(move || {
        spawn(async move {
            async fn connect(mut radio: Radio<DevtoolsState, DevtoolsChannel>) -> Result<(), tungstenite::Error> {
                let address = devtools_addr();
                let websocket_url = format!("ws://{address}");
                println!("Connecting to devtools server at {address}");
                let tcp_stream = TcpStream::connect(address.as_str()).await?;
                let (ws_stream, _response) = async_tungstenite::client_async(websocket_url, tcp_stream).await?;
            ...
            loop {
                println!("Waiting for a devtools server...");
                connect(radio).await.ok();
                radio.write_channel(DevtoolsChannel::UpdatedTree).nodes.clear();
...
impl Component for LayoutForNodeInspector {
    fn render(&self) -> impl IntoElement {
        ...
        let area = format!("{}x{}", node_info.area.width().round(), node_info.area.height().round());
        ...
        rect()
            .expanded()
            .child(
                ScrollView::new().show_scrollbar(false).height(Size::px(280.)).child(
                    rect()
                        .padding(16.)
                        .width(Size::fill())
                            ...
                            rect()
                                ...
                                .child(
                                    rect()
                                        ...
                                        .child(info_label(node_info.children_len.to_string(), "children"))
                                        .child(info_label(node_info.layer.to_string(), "layer")),
                                )
            ...
            .child(
                ScrollView::new().show_scrollbar(false).height(Size::auto()).child(
                    rect()
                        .direction(Direction::Horizontal)
                        .padding((0., 4.))
                        .child(inspector_tab(Route::NodeInspectorStyle { node_id, window_id }, "Style"))
                        .child(inspector_tab(
...
impl Component for LayoutForTreeInspector {
    fn render(&self) -> impl IntoElement {
        ...
        ResizableContainer::new()
            .direction(Direction::Horizontal)
            .panel(
                ResizablePanel::new(PanelSize::percent(60.)).child(rect().padding(10.).child(NodesTree {
                    selected_node_id,
                    selected_window_id,
                    on_selected: EventHandler::new(move |(window_id, node_id)| {
                        radio
                            .read()
                            .send_action(IncomingMessageAction::HighlightNode { window_id, node_id });
                    }),
                    on_hover: EventHandler::new(move |(window_id, node_id)| {
                        radio
                            .read()
                            .send_action(IncomingMessageAction::HoverNode { window_id, node_id });
                    }),
                })),
            )
