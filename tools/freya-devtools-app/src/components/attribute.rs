...
use crate::property::{FillProperty, Property};

pub fn attributes_list(attributes: Vec<(&str, AttributeType)>) -> Element {
    ScrollView::new()
        .children(attributes.into_iter().enumerate().map(|(i, (name, attribute))| {
...
pub fn attribute_element(name: &str, attribute: AttributeType<'_>) -> Element {
    match attribute {
        ...
        AttributeType::VisibleSize(visible_size) => Property::new(name, visible_size.pretty()).into(),
        ...
        AttributeType::TextOverflow(text_overflow) => Property::new(name, text_overflow.pretty()).into(),
        AttributeType::Length(length) => Property::new(name, length.get().to_string()).into(),
        AttributeType::TextHeightBehavior(text_height) => Property::new(name, text_height.pretty()).into(),
        AttributeType::FontSlant(font_slant) => Property::new(name, font_slant.pretty()).into(),
        AttributeType::TextDecoration(text_decoration) => Property::new(name, text_decoration.pretty()).into(),
    }
}
